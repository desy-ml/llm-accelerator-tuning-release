...
class TransverseTuning(gym.Env):
    """
    Transverse beam parameter tuning environment for the ARES bunch commpressor section.

    Magnets: ARMRMQZM4, ARMRMQZM5, ARMRMCVM5, ARMRMCHM5 and ARMRMQZM6
    Screen: ARBCBSCE1
    ...
    def __init__(
        ...
        if unidirectional_quads:
            self._magnet_space = spaces.Box(
                low=np.array(
                    [0, -max_quad_setting, -6.1782e-3, -6.1782e-3, 0], dtype=np.float32
                ),
                high=np.array(
                    [max_quad_setting, 0, 6.1782e-3, 6.1782e-3, max_quad_setting],
                    dtype=np.float32,
                ),
            )
        else:
...
class CheetahBackend(TransverseTuningBaseBackend):
    ...
    def __init__(
        ...
        # Simulation setup
        ocelot_cell = (
            ocelot_lattice.drift_armrbscr1,
            ocelot_lattice.armrmqzm4,
            ocelot_lattice.drift_armrmqzm4,
            ocelot_lattice.armreolt1,
            ocelot_lattice.ardgsolo1,
            ocelot_lattice.drift_ardgsolo1,
            ocelot_lattice.ardgeolo1,
            ocelot_lattice.armrsolb1,
            ocelot_lattice.drift_armrsolb1,
            ocelot_lattice.armrbpmg3,
            ocelot_lattice.drift_armrbpmg3,
            ocelot_lattice.armrmqzm5,
            ocelot_lattice.drift_armrmqzm5,
            ocelot_lattice.armrmcvm5,
            ocelot_lattice.drift_armrmcvm5,
            ocelot_lattice.armrmchm5,
            ocelot_lattice.drift_armrmchm2,
            ocelot_lattice.armrbscr3,
            ocelot_lattice.drift_armrbscr1,
            ocelot_lattice.armrmqzm6,
            ocelot_lattice.drift_armrmqzm6,
            ocelot_lattice.armreolb1,
            ocelot_lattice.arbcsolc,
            ocelot_lattice.drift_arbcsolc,
            ocelot_lattice.arbcmbhb1,
            ocelot_lattice.drift_arbcmbhb1,
            ocelot_lattice.arbcmbhb2,
            ocelot_lattice.drift_arbcmbhb2,
            ocelot_lattice.arbcbpml1,
            ocelot_lattice.drift_arbcbpml1,
            ocelot_lattice.arbcslhb1,
            ocelot_lattice.drift_arbcslhb1,
            ocelot_lattice.arbcslhs1,
            ocelot_lattice.drift_arbcslhs1,
            ocelot_lattice.arbcbsce1,
        )
        self.segment = cheetah.Segment.from_ocelot(
            ocelot_cell, warnings=False, device="cpu"
        )

        self.segment.ARBCBSCE1.resolution = torch.tensor((2463, 2055))
        self.segment.ARBCBSCE1.pixel_size = torch.tensor((6.0000e-6, 4.1200e-6))
        self.segment.ARBCBSCE1.binning = torch.tensor(1)
    ...
    def set_magnets(self, values: Union[np.ndarray, list]) -> None:
        self.segment.ARMRMQZM4.k1 = torch.tensor(values[0], dtype=torch.float32)
        self.segment.ARMRMQZM5.k1 = torch.tensor(values[1], dtype=torch.float32)
        self.segment.ARMRMCVM5.angle = torch.tensor(values[2], dtype=torch.float32)
        self.segment.ARMRMCHM5.angle = torch.tensor(values[3], dtype=torch.float32)
        self.segment.ARMRMQZM6.k1 = torch.tensor(values[4], dtype=torch.float32)
    ...
    def reset(self, options=None) -> None:
        ...
        self.segment.ARMRMQZM4.misalignment = torch.as_tensor(misalignments[0:2])
        self.segment.ARMRMQZM5.misalignment = torch.as_tensor(misalignments[2:4])
        self.segment.ARMRMQZM6.misalignment = torch.as_tensor(misalignments[4:6])
        self.segment.ARBCBSCE1.misalignment = torch.as_tensor(misalignments[6:8])
    ...
    def get_beam_parameters(self) -> np.ndarray:
        if self.simulate_finite_screen and not self.is_beam_on_screen():
            ...
            read_beam = self.segment.ARBCBSCE1.get_read_beam()
            return np.array(
    ...
    def get_screen_image(self) -> np.ndarray:
        ...
        # scaling to 12 bits)
        return (self.segment.ARBCBSCE1.reading).numpy() / 1e9 * 2**12
...
class DOOCSBackend(TransverseTuningBaseBackend):
    ...
    screen_channel = "SINBAD.DIAG/CAMERA/AR.BC.BSC.E.1"
    magnet_channels = [
        "SINBAD.MAGNETS/MAGNET.ML/ARMRMQZM4/STRENGTH",
        "SINBAD.MAGNETS/MAGNET.ML/ARMRMQZM5/STRENGTH",
        "SINBAD.MAGNETS/MAGNET.ML/ARMRMCVM5/KICK",
        "SINBAD.MAGNETS/MAGNET.ML/ARMRMCHM5/KICK",
        "SINBAD.MAGNETS/MAGNET.ML/ARMRMQZM6/STRENGTH",
    ]
