...
class TransverseTuning(gym.Env):
    """
    Transverse beam parameter tuning environment for the ARES SH section.
    ...
    def __init__(
        ...
        if unidirectional_quads:
            self._magnet_space = spaces.Box(
                low=np.array(
                    [-6.1782e-3, 0, -6.1782e-3, -max_quad_setting], dtype=np.float32
                ),
                high=np.array(
                    [6.1782e-3, max_quad_setting, 6.1782e-3, 0], dtype=np.float32
                ),
            )
        else:
            self._magnet_space = spaces.Box(
                low=np.array(
                    [-6.1782e-3, -max_quad_setting, -6.1782e-3, -max_quad_setting],
                    dtype=np.float32,
                ),
                high=np.array(
                    [6.1782e-3, max_quad_setting, 6.1782e-3, max_quad_setting],
    ...
    def _get_info(self):
        return {
            ...
            "magnet_names": ["ARDLMCVM2", "ARDLMQZM3", "ARDLMCHM2", "ARDLMQZM4"],
...
class CheetahBackend(TransverseTuningBaseBackend):
    ...
    def __init__(
        ...
        # Simulation setup
        ocelot_cell = (
            ocelot_lattice.ardlmcvm2,
            ocelot_lattice.drift_armrmqzm1,
            ocelot_lattice.ardlmqzm3,
            ocelot_lattice.drift_ardlmqzm3,
            ocelot_lattice.ardlmchm2,
            ocelot_lattice.drift_armrmqzm1,
            ocelot_lattice.ardlmqzm4,
            ocelot_lattice.drift_ardlmqzm4,
            ocelot_lattice.ardleolm1,
            ocelot_lattice.arshsolh1,
            ocelot_lattice.drift_arshsolh1,
            ocelot_lattice.arshmbho1,
            ocelot_lattice.drift_arshmbho1,
            ocelot_lattice.arshbsce2,
        )
        self.segment = cheetah.Segment.from_ocelot(
            ocelot_cell, warnings=False, device="cpu"
        )

        self.segment.ARSHBSCE2.resolution = torch.tensor((2464, 2056))
        self.segment.ARSHBSCE2.pixel_size = torch.tensor((4.7650e-6, 3.3720e-6))
        self.segment.ARSHBSCE2.binning = torch.tensor(1)
    ...
    def set_magnets(self, values: Union[np.ndarray, list]) -> None:
        self.segment.ARDLMCVM2.angle = torch.tensor(values[0], dtype=torch.float32)
        self.segment.ARDLMQZM3.k1 = torch.tensor(values[1], dtype=torch.float32)
        self.segment.ARDLMCHM2.angle = torch.tensor(values[2], dtype=torch.float32)
        self.segment.ARDLMQZM4.k1 = torch.tensor(values[3], dtype=torch.float32)
    ...
    def reset(self, options=None) -> None:
        ...
        self.segment.ARDLMQZM3.misalignment = torch.as_tensor(misalignments[0:2])
        self.segment.ARDLMQZM4.misalignment = torch.as_tensor(misalignments[2:4])
        self.segment.ARSHBSCE2.misalignment = torch.as_tensor(misalignments[4:6])
    ...
    def get_beam_parameters(self) -> np.ndarray:
        if self.simulate_finite_screen and not self.is_beam_on_screen():
            ...
            read_beam = self.segment.ARSHBSCE2.get_read_beam()
            return np.array(
    ...
    def get_screen_image(self) -> np.ndarray:
        ...
        # scaling to 12 bits)
        return (self.segment.ARSHBSCE2.reading).numpy() / 1e9 * 2**12
...
class DOOCSBackend(TransverseTuningBaseBackend):
    """
    Backend for the ARES SH section to communicate with the real accelerator through the
    ...
    screen_channel = "SINBAD.DIAG/CAMERA/AR.SH.BSC.E.2"
    magnet_channels = [
        "SINBAD.MAGNETS/MAGNET.ML/ARDLMCVM2/KICK",
        "SINBAD.MAGNETS/MAGNET.ML/ARDLMQZM3/STRENGTH",
        "SINBAD.MAGNETS/MAGNET.ML/ARDLMCHM2/KICK",
        "SINBAD.MAGNETS/MAGNET.ML/ARDLMQZM4/STRENGTH",
    ]
