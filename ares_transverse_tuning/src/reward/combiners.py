...
Combiners are a special type of transform that reduces a vector of values down to a
single value. The combiner can be skipped if the target only outputs a single value, or
if you want a vector of outputs for the final combiner.

NOTE: This file was copied and modified from Deepmind's fusion paper code:
https://github.com/deepmind/deepmind-research/blob/master/fusion_tcv/combiners.py
"""
...
class AbstractCombiner(abc.ABC):
    ...
    def _clean_values_weights(
        ...
        if weights is None:
            ...
            if len(values) != len(weights):
                raise ValueError(
                    "Number of weights don't match values. "
        ...
        new_values_weights = [
            (v, w) for v, w in zip(values, weights) if not np.isnan(v) and w > 0
        ]
...
class SmoothMax(AbstractCombiner):
    ...
    def __call__(
        ...
        log_soft_weights = [np.log(w) + c * self.alpha for w, c in zip(weights, values)]
