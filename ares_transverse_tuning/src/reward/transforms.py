...
Transforms can make arbitrary conversions, but one of them must change from the
arbitrary (often meaningful) scale to a reward in the 0-1 range.

NOTE: This file was copied and modified from Deepmind's fusion paper code:
https://github.com/deepmind/deepmind-research/blob/master/fusion_tcv/transforms.py
"""
...
@dataclasses.dataclass(frozen=True)
class Linear(AbstractTransform):
    """Scales errors, bad to 0, good to 1. Does not guarantee 0-1."""

    bad: float
    good: float = 0

    def __call__(self, errors: List[float]) -> List[float]:
        return [scale(err, self.bad, self.good, 0, 1) for err in errors]
...
class ClippedLinear(AbstractTransform):
    ...
    def __call__(self, errors: List[float]) -> List[float]:
        return [clip(scale(err, self.bad, self.good, 0, 1), 0, 1) for err in errors]
...
class SoftPlus(AbstractTransform):
    ...
    def __call__(self, errors: List[float]) -> List[float]:
        return [
            clip(2 * logistic(scale(e, self.bad, self.good, self.low, 0)), 0, 1)
            for e in errors
        ]
...
class NegExp(AbstractTransform):
    ...
    def __call__(self, errors: List[float]) -> List[float]:
        return [
            clip(math.exp(-scale(e, self.bad, self.good, self.low, 0)), 0, 1)
            for e in errors
        ]
...
class Sigmoid(AbstractTransform):
    ...
    def __call__(self, errors: List[float]) -> List[float]:
        return [
            logistic(scale(err, self.bad, self.good, self.low, self.high))
            for err in errors
        ]
