...
from src.environments import sh
...
def optimize(
    ...
    """
    Optimise beam in ARES SH using a reinforcement learning agent.
    """
    config = load_config(f"../models/sh/ppo/{model_name}/config")

    # Load the model
    model = PPO.load(f"../models/sh/ppo/{model_name}/model")
