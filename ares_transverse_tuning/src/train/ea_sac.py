...
def train(config: dict) -> None:
    ...
    model.save(f"models/ea/sac/{wandb.run.name}/model")
    if (config["normalize_observation"] and config["running_obs_norm"]) or config[
        "normalize_reward"
    ]:
        vec_env.save(f"models/ea/sac/{wandb.run.name}/vec_normalize.pkl")
    save_config(config, f"models/ea/sac/{wandb.run.name}/config")
