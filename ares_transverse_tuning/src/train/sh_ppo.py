...
def main() -> None:
    config = {
        ...
        "magnet_change_combiner_weights": [1, 1, 1, 1],
        "final_combiner": "Mean",
        "final_combiner_args": {},
...
def train(config: dict) -> None:
    ...
    model.save(f"models/sh/ppo/{wandb.run.name}/model")
    if (config["normalize_observation"] and config["running_obs_norm"]) or config[
        "normalize_reward"
    ]:
        vec_env.save(f"models/sh/ppo/{wandb.run.name}/vec_normalize.pkl")
    save_config(config, f"models/sh/ppo/{wandb.run.name}/config")
