...
def train(config: dict) -> None:
    ...
    # Make sure max_travel_distances is a list
    if not isinstance(config["optimizer_kwargs"]["max_travel_distances"], list):
        config["optimizer_kwargs"]["max_travel_distances"] = [
            config["optimizer_kwargs"]["max_travel_distances"]
        ] * len(config["action_order"])
    ...
    dfs = []
    for i in tqdm.trange(config["n_testepisodes"]):
        # Reset Agent
    ...
    # Save data
    save_dir = f"models/tune_optimizer/{config['method']}/{wandb.run.name}"
